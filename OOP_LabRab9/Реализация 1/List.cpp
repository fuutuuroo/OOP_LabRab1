...
List::List(int s) {
	if (s > MAX_SIZE) {				//обработка превышения максимального размера
		throw 1;
	}
	size = s;
	first = nullptr;
...
void List::push_back(int data) {
	if (size + 1 == MAX_SIZE) {
		throw 1;					//обработка превышения максимального размера
	}
	if (first == nullptr) {
		first = new Node(data);
	}
...
void List::pop_front() {
	if (first == nullptr) {
		throw exception("Список пуст!");	//обработка случая с пустым списком
	}
	Node* temp = first;
	first = first->pNext;
	delete temp;
...
int& List::operator [] (int index) {
	if (index < 0)
		throw 2;				//обработка индекса меньше нуля
	if (index >= size)
		throw 1;				//обработка индекса больше размера
...
void List::operator ++ () {
	if (size + 1 == MAX_SIZE) {
		throw 3;				//обработка превышения максимального размера
	}
	if (first == nullptr) {
		first = new Node();
	}
